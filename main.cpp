...
void saveJobs()
{
    ...
    // Jobs are stored in PostgreSQL, not the Render container filesystem.
    // This means listings and their removed state survive redeploys/restarts.
    if (!dbExec("BEGIN;")) return;
...
)
{
    ...
    )
    {
        ...
        if (job)
        {
            ...
            // Persist the removal immediately in PostgreSQL.
            // saveJobs() also writes the full in-memory job state.
            if (!dbExecParams(
                "UPDATE jobs SET removed=TRUE WHERE id=$1;",
                {std::to_string(job->id)}
            ))
            {
                std::cerr << "Failed to persist job removal for job "
                          << job->id << "\n";
            }
